...
export const returnBorrowBook = catchAsyncErrors(async (req, res, next) => {
  ...
  borrow.fine = fine;
  await borrow.save();
  const message = fine !== 0 ? `The book has returned successfully.The total charges, including a fine, are $${fine+book.price}` : `The book has returned successfully.The total charges are $${book.price}`;
  res.status(200).json({
      success: true,
      message,
  });
